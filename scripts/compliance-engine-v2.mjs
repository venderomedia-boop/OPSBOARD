...
export function createSiteAssignment(input){if(!site(input?.siteId)||!formType(input?.formTypeId))throw Object.assign(new Error('valid siteId and formTypeId are required'),{statusCode:422});if(input.jobTemplateId&&!template(input.jobTemplateId))throw Object.assign(new Error('invalid jobTemplateId'),{statusCode:422});const item={id:input.id||makeId('assign',`${input.siteId}-${input.formTypeId}`),siteId:input.siteId,formTypeId:input.formTypeId,jobTemplateId:input.jobTemplateId||null,frequency:input.frequency||formType(input.formTypeId).frequency||'ad_hoc',active:input.active!==false,nextDueDate:input.nextDueDate||null};state.siteAssignments.push(item);persist();return clone(item);}
