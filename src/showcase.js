...
document.addEventListener('click',routeClick,true);document.addEventListener('keydown',routeKey,true);document.addEventListener('input',updateWarnings,true);const showcaseObserver=new MutationObserver(()=>{showcaseObserver.disconnect();enhanceSurface();showcaseObserver.observe(document.documentElement,{childList:true,subtree:true});});showcaseObserver.observe(document.documentElement,{childList:true,subtree:true});setTimeout(async()=>{enhanceSurface();const reopen=sessionStorage.getItem(REOPEN_KEY);if(reopen){sessionStorage.removeItem(REOPEN_KEY);try{await getOverview();await openComplianceJob(reopen);}catch{}}},150);
